...
def random_forest(df, year, ccy, model_output_filepath):
    ...
    results_df.to_csv(rf'{model_output_filepath}/{year}_{ccy}_random_forest_results.csv', index=False)
    df.to_csv(rf'{model_output_filepath}/{year}_{ccy}_random_forest_dataframe.csv')
